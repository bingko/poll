    ...
    <table width="90%" align="center" cellpadding="2" cellspacing="2" class="survey_layer">
      <tr>
        ...
        <?php } ?>       
            ...
            <input name="3" type="radio" <?php echo @$posetion1?>/>นักศึกษา
            <input name="3" type="radio" <?php echo @$posetion2?>/>อาจารย์
            <input name="3" type="radio" <?php echo @$posetion3?>/>บุคลากรสายสนับสนุน
            <input name="3" type="radio" <?php echo @$posetion4?>/>บุคคลทั่วไป<p>
            <?php }if(@$_SESSION['data_respondent'][$i]==4){?>
            <p><strong>รายได้ส่วนตัวเฉลี่ยนต่อเดือน</strong>
            <input name="4" type="radio" <?php echo @$monny1?>/>ไม่เกิน 5,000 บาท
            <input name="4" type="radio" <?php echo @$monny2?>/>5,001 - 10,000 บาท
            <input name="4" type="radio" <?php echo @$monny3?>/>10,001 - 15,000 บาท
            <input name="4" type="radio" <?php echo @$monny4?>/>15,001 - 20,000 บาท
            <input name="4" type="radio" <?php echo @$monny5?>/>20,001 - 25,000 บาท
            <input name="4" type="radio" <?php echo @$monny6?>/>25,001 บาทขึ้นไป<p>
            <?php }if(@$_SESSION['data_respondent'][$i]==5){?>
            <p><strong>การศึกษา</strong>
            <input name="5" type="radio" <?php echo @$education1?>/>ต่ำกว่า ม.6
            <input name="5" type="radio" <?php echo @$education2?>/>ปริญาตรี
            <input name="5" type="radio" <?php echo @$education3?>/>ปริญาโท
            <input name="5" type="radio" <?php echo @$education4?>/>ปริญาเอก<p>
      ...
      <tr>
		...
		<?php for($i=0;$i<count(@$_SESSION['qa_all']);$i++){?>        
			...
			if(@$_SESSION['qa_all'][$i]['type']=="mulitiple_only"){ 
				...
				$query = $this->db->get('ans_value');
				foreach ($query->result() as $mulitiple_multiple_row)
